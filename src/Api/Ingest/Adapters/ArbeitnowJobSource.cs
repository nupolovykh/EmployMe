    ...
    IOptions<IngestOptions> options) : IJobSource
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        for (var page = 0; page < options.Value.MaxPagesPerSource && url is not null; page++)
        {
            ...
            // The next link comes out of the upstream response, so it is not
            // trusted to point back at the upstream: an origin change would have
            // us fetching an arbitrary host and storing its body in raw_postings.
            var next = document.RootElement.TryGetProperty("links", out var links)
                && links.ValueKind == JsonValueKind.Object
                    ? links.String("next")
                    : null;

            url = SameOrigin(next, baseUrl) ? next : null;
        }
    }
    ...
    private static bool SameOrigin(string? candidate, string baseUrl) =>
        ...
        && string.Equals(next.Host, origin.Host, StringComparison.OrdinalIgnoreCase)
